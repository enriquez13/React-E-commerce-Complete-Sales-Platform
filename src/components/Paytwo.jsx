...
const Paytwo = () => {
  ...
  const onSubmit = async (data) => {
    ...
    let orden = {
        cliente:{ 
            ...
            producto: cart.map((product, index)=> ({key: index, Ide:product.ide, Id:product.id, Nombre: product.nombre, Talla: product.talla, Color: product.color, Precio: product.valor, Cantidad: product.quantity})),
            envio: data.option,
            valorenvio: valorEnvio(),
            fecha: new Date().toISOString(),
            total: total(),
  ...
  return (
    ...
    <form onSubmit={handleSubmit(onSubmit)} className='mx-7 text-black  md:mx-[20rem]'>
      ...
      <h3 className='col-span-6 text-center py-2'>Elija el tipo de envío:</h3>
            <div className='col-span-5'>
              ...
              {errors.option && errors.option.type === 'required' && <p className='text-red-600 bg-red-200 rounded-lg pl-2'>Seleccion la opción o forma de envío</p>}
            </div>
      ...
      <div className='text-red-600 bg-red-200 mb-3 pl-2 rounded-lg'>{errors.email && errors.email.type === 'required' && <p>Falta escribir el correo</p>}
      {errors.email && errors.email.type === 'minLength' && <p>El correo debe tener al menos 4 caracteres</p>}
      {errors.email && errors.email.type === 'pattern' && <p>El correo no puede contener símbolos diferentes de @_.</p>}
      </div>
      <label htmlFor="name">Nombres:</label>
      <input id="name" type="text" {...register('name', { required: true, minLength: 4, pattern: /^[a-zA-ZáéíóúñÁÉÍÓÚ\s]+$/  })} 
      className='w-full mt-1 bg-gray-100'/>
      <div className='text-red-600 bg-red-200 mb-3 pl-2 rounded-lg'>{errors.name && errors.name.type === 'required' && <p>Falta escribir el nombre</p>}
      ...
      className='w-full mt-1  bg-gray-100'/>
      <div className='text-red-600 bg-red-200 mb-3 pl-2 rounded-lg'>{errors.apellido && errors.apellido.type === 'required' && <p>Falta escribir el apellido</p>}
      ...
      className='w-full mt-1  bg-gray-100'/>
      <div className='text-red-600 bg-red-200 mb-3 pl-2 rounded-lg'>{errors.cedula && errors.cedula.type === 'required' && <p>Falta escribir el número de cédula o pasaporte</p>}
      ...
      className='w-full mt-1 bg-gray-100'/>
      <div className='text-red-600 bg-red-200 mb-3 pl-2 rounded-lg'>{errors.telefono && errors.telefono.type === 'required' && <p>Falta escribir el teléfono</p>}
      ...
      className='w-full mt-1 bg-gray-100'/>
      <div className='text-red-600 bg-red-200 mb-3 pl-2 rounded-lg'>{errors.ciudad && errors.ciudad.type === 'required' && <p>Falta escribir la ciudad</p>}
      ...
      className='w-full mt-1 bg-gray-100'/>
      <div className='text-red-600 bg-red-200 mb-3 pl-2 rounded-lg'>{errors.direccion && errors.direccion.type === 'required' && <p>Falta escribir la dirección</p>}
      ...
      <button type="submit" className="rounded-lg mt-5 bg-black text-white px-10 md:px-[7rem] py-2 md:py-3">
      Finalizar
      </button>
      </div>
    </form>
